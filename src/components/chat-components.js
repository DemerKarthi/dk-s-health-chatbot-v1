...
export function ChatComponent({ activeTopic }) {
  const [messages, setMessages] = useState([
    {
      id: "welcome-message",
      role: "assistant",
      content: "Hi there! I'm  your wellness assistant.Created by Dhanakarthikeyan !",
    },
  ]);
  ...
  return (
    <div className="flex-1 flex flex-col h-full relative">
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-blue-50 pb-20">
        {messages.map((message) => (
          <div key={message.id} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
            <div className="flex items-start gap-3 max-w-[85%]">
              ...
              {message.role === "user" && (
                <Avatar className="mt-1 flex-shrink-0">
                  <AvatarFallback className="bg-gray-400 text-white"></AvatarFallback>
                </Avatar>
              )}
            </div>
          </div>
